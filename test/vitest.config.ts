...
export default defineConfig({
  test: {
    ...
    env: envFromFile,
    include: ['src/**/*.test.ts'],
    exclude: ['**/node_modules/**', 'src/test/probe-funding.test.ts'],
    reporters: ['default'],
    sequence: { concurrent: false },
    // Multiple test files share the same devnet wallets/seeds; run files one
    // at a time so their wallets never submit concurrent transactions.
